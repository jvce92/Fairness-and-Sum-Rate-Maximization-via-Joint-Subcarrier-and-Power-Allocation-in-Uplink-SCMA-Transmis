...
class MaxMinPowerOptimizer(BaseOptimizer):
        ...
        for j in range(self.J):
            ...
            if j == 0:
                constraints.append(
                    cvx.sum(
                        cvx.log(
                            1 + cvx.multiply(cvx.multiply(self.rho[:, j], self.Fk[:, j]), self.P[:, j]))
                    ) >= self.z
                )
            else:
                constraints.append(
                    cvx.sum(
                        cvx.log(1 + cvx.sum(cvx.multiply(cvx.multiply(self.rho[:, :j+1], self.Fk[:, :j+1]), self.P[:, :j+1]), axis=1)) -
                        cvx.log(1 + cvx.sum(cvx.multiply(cvx.multiply(self.rho[:, :j], self.Fk[:, :j]), self.Pk[:, :j]), axis=1)) -
                        cvx.multiply(cvx.multiply(cvx.multiply(self.Fk[:, j], self.Pk[:, j]),
                                                  cvx.power((1 + cvx.sum(cvx.multiply(cvx.multiply(self.rho[:, :j], self.Fk[:, :j]), self.Pk[:, :j]))), -1)),
                                     (self.P[:, j] - self.Pk[:, j]))
                    ) >= self.z
                )
        self.prob = cvx.Problem(cvx.Maximize(self.z), constraints)
    ...
    def inner_step(self):
        ...
        phi_prev = np.min(
            np.sum(rate(self.Fk.value, self.Pk_prev, self.H, self.N0), axis=0))
        phi_new = np.min(
            np.sum(rate(self.Fk.value, self.P.value, self.H, self.N0), axis=0))
        sigma_hat = self.z.value - phi_prev
    ...
    def step(self):
        while self.niter < self.max_it and np.abs(self.z_val - self.zk_prev) > self.tol and self.iter_no_improve < self.max_no_improve:
            self.niter += 1
        ...
        return Status(obj=self.z_val, status=status, variable=self.Pk.value)

    def solve(self):
        raise NotImplementedError("Not implemented in the power optimizer")


class MaxMinSubcarrierOptimizer(BaseOptimizer):
    def __init__(self, K, J, **kwargs):
        super().__init__(K, J, **kwargs)
        self.z = cvx.Variable(nonneg=True)
        ...
        for j in range(self.J):
            ...
            if j == 0:
                constraints.append(
                    cvx.sum(
                        cvx.log(
                            1 + cvx.multiply(cvx.multiply(self.rho[:, j], self.F[:, j]), self.Pk[:, j]))
                    ) >= self.z
                )
            else:
                constraints.append(
                    cvx.sum(
                        cvx.log(1 + cvx.sum(cvx.multiply(cvx.multiply(self.rho[:, :j+1], self.Pk[:, :j+1]), self.F[:, :j+1]), axis=1)) -
                        cvx.log(1 + cvx.sum(cvx.multiply(cvx.multiply(self.rho[:, :j], self.Fk[:, :j]), self.Pk[:, :j]), axis=1)) -
                        cvx.multiply(cvx.multiply(cvx.multiply(self.Fk[:, j], self.Pk[:, j]),
                                                  cvx.power((1 + cvx.sum(cvx.multiply(cvx.multiply(self.rho[:, :j], self.Fk[:, :j]), self.Pk[:, :j]))), -1)),
                                     (self.F[:, j] - self.Fk[:, j]))
    ...
    def inner_step(self):
        ...
        phi_new = np.min(np.sum(rate(self.F.value, self.Pk.value, self.H, self.N0),
                                axis=0)) + self.lam.value * np.sum(self.F.value**2 - self.F.value)
...
class JointMaxMinOptimizer:
    def __init__(self, K, J, **kwargs):
        ...
        self.pwr_opt = MaxMinPowerOptimizer(
            K, J,
            max_it=max_it,
            tol=pwr_tol,
        ...
        self.sc_opt = MaxMinSubcarrierOptimizer(
            K, J,
            max_it=max_it,
            tol=sc_tol,
            max_no_improve=max_no_improve,
